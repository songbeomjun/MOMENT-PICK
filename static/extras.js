...
document.addEventListener('DOMContentLoaded', function() {
  // 데일리 이벤트 체크
  setTimeout(checkDailyEvent, 2000);
...
function openSlot() {
  ...
  if (btn) btn.disabled = used === today;
  if (btn && used === today) btn.textContent = '오늘은 이미 뽑았어요 · 내일 다시 오세요!';
  else if (btn) btn.textContent = '🎰 행운을 뽑아라!';
  initSlotReels();
  document.getElementById('slotOverlay').classList.add('show');
}
function closeSlot() { document.getElementById('slotOverlay').classList.remove('show'); }
